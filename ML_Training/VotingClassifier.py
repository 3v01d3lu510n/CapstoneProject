...
# Other features
other_features = df[['InfoEntropy', 'SpecialCharEntropy', 'QuoteEntropy', 'characteristics_flag']].astype(float).fillna(0)

# Combine all features
X = pd.concat([other_features, tfidf_cols], axis=1)
X.columns = X.columns.map(str)  # Ensure all column names are strings
...
clf3 = ExtraTreesClassifier(n_estimators=100, random_state=42, n_jobs=-1)
